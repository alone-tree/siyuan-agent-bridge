...
当前 Agent 环境提供 CodeGraph MCP 工具，分析代码结构、查找符号、追踪调用链、评估改动影响时必须优先使用 CodeGraph。调用subagent也需要明确告知优先使用codegraph。

- 整体理解、架构分析、功能流向：先用 `codegraph_explore`。
- 查找符号位置：先用 `codegraph_search`。
- 查调用方、被调用方、影响范围：使用 `codegraph_callers`、`codegraph_callees`、`codegraph_impact`。
- 查看索引状态或文件树：使用 `codegraph_status`、`codegraph_files`。
- CodeGraph 没覆盖、结果不足或文件刚修改且索引未同步时，再用 `rg`、`type` 或直接读文件补充确认。

### CodeGraph 注册

如果当前 Agent 环境没有暴露 `codegraph_*` 工具，先确认是否已安装并注册 CodeGraph MCP。

官方 CodeGraph 包是 `@colbymchenry/codegraph`，MCP 启动命令是：
...
```bat
cmd /d /s /c "chcp 65001 >nul && npm install -g @colbymchenry/codegraph"
cmd /d /s /c "chcp 65001 >nul && cd /d D:\Github\siyuan-bridge && codegraph init"
cmd /d /s /c "chcp 65001 >nul && cd /d D:\Github\siyuan-bridge && codegraph status"
```

Codex 全局配置文件通常是 `%USERPROFILE%\.codex\config.toml`。如果安装器没有自动写入，可以手动加入：
...
也可以先查看 CodeGraph 给 Codex 的官方配置片段：

```bat
cmd /d /s /c "chcp 65001 >nul && codegraph install --print-config codex"
```

注册或修改 MCP 配置后，需要重启 Codex 或开启新会话，当前已启动会话通常不会热加载新增 MCP 工具。当前仓库已在 `.gitignore` 中忽略 `/.codegraph`，不要提交本地索引目录。
