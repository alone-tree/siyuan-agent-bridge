...
- 刷新安全索引，确保数据是最新的。
- 检查思源本地服务是否可用。
- 返回入口材料，包括笔记本概览表、`START_HERE.md` 和 `knowledge_base/guide.md`。
...
- `siyuan_start`：刷新安全索引并返回启动包（含笔记本概览表、START_HERE.md、guide.md）。始终最先调用。
- `siyuan_refresh_index`：在用户明确要求时，在会话中途刷新安全索引（siyuan_start 已在启动时刷新）。
- `siyuan_list_notebooks`：列出安全索引里的可见笔记本。
- `siyuan_list_documents`：返回某个笔记本的文档树，含字数和更新时间。
- `siyuan_find_documents`：搜索知识库，支持 4 种模式（`keyword`/`query`/`regex`/`sql`）、2 种范围（`headings`/`full`），可选限定笔记本。
