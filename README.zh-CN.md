...
- `kb_cache/guide.md`：告诉 AI 哪些笔记本、路径、主题最重要。
...
- 生成 AI 可读的索引文件，例如 `kb_cache/tree.md` 和 `kb_cache/docs.jsonl`。
...
AI 会负责重新扫描并清理旧索引。之前已经暴露在 `kb_cache/` 里的文档，如果现在被隐藏，会从新索引里移除。
...
重要的是：临时开放不会重写长期索引 `kb_cache/`，所以临时打开的内容不会长期留在 AI 可见目录里。
...
1. 先读 `AGENTS.md`。
2. 再读 `kb_cache/guide.md`。
3. 再读 `kb_cache/tree.md`。
...
- `kb_cache/` 和 `ai_workspace/` 默认可以进入 Git，因为当前仓库是私人使用。
