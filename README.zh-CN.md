...
如果你在其他 AI 工具里使用这个项目，最推荐告诉它：

```text
请先阅读 D:\Github\siyuan-enhance\START_HERE.md。
如果有 MCP 工具，请先调用 siyuan_start。它只检查思源连接并返回现有入口材料，不会刷新索引。
如果没有 MCP，请在 D:\Github\siyuan-enhance 运行 python -m source_code start。
```
...
- 生成启动总览 `knowledge_base/overview.md`。
- 生成每个笔记本自己的地图 `knowledge_base/notebooks/<notebook-id>.md`。
...
1. 先读 `START_HERE.md`。
2. 优先调用 MCP 工具 `siyuan_start`，它只检查连接并返回现有顶层索引。
3. 没有 MCP 时运行 `python -m source_code start`。
4. 根据启动包和 `knowledge_base/overview.md` 选择相关笔记本。
5. 需要时读取 `knowledge_base/notebooks/<notebook-id>.md`。
6. 需要深入时，按文档 ID 读取具体文档。
7. 生成的分析和草稿放进 `ai_workspace/`。
8. 只有当你明确说已经改了 ignore、索引缺失或明显过期时，AI 才应该刷新索引。
