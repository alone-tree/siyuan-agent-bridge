...
| `siyuan_find_documents` | fullTextSearchBlock + 隐私过滤 | FTS5 全文搜索，4 种模式 |
| `siyuan_read_document` | getBlockTree + exportMdContent + getBlockInfo | 大纲 + 窗口分段 + 附件提取 |
| `siyuan_edit_document` | getBlockInfo + updateBlock + createSnapshot | 文本锚点替换，单块编辑 |
| `siyuan_create_document` | createDocWithMd + createSnapshot | Markdown 新建，支持标签/模板 |

另外 4 个辅助工具：`siyuan_start`（启动入口）、`siyuan_refresh_index`（手动刷新索引）、`siyuan_propose_guide_update`、`siyuan_apply_guide_update`（AI 指南管理）。
...
思源支持跨文档块引用语法 `((block_id "锚文本"))`。`siyuan_read_document` 提供 `include_block_ids` 参数（默认 `false`），开启后对外称"引用阅读"模式。

**引用阅读的实现**：通过 `/api/block/getChildBlocks` 按思源返回的真实子块顺序递归遍历块树，在 Markdown 中注入 `<!-- siyuan:block id=... type=... -->` HTML 注释：

```markdown
## 中际旭创分析 <!-- siyuan:block id=20260410083015-abc123 type=h -->

2025年营收增长45%，其中800G光模块占比超过60%。
<!-- siyuan:block id=20260410083016-def456 type=p -->
```

AI 读到干净的 Markdown，HTML 注释不影响理解。需要跨文档引用时，AI 从注释中提取目标块 ID，构造 `((20260410083016-def456 "800G光模块占比超60%"))`，思源笔记会自动将其识别为对应的引用。若ID错误，思源笔记会报错，提示ID不存在。

**遍历规则**：
- 跳过文档块、列表容器块和空块
- 列表项、表格这类自身 Markdown 已包含子内容的块，渲染后不再递归子孙，避免重复
- 超级块只显示块 ID 注释并继续遍历子块

**默认不启用**：普通阅读保持纯 Markdown 体验。块 ID 嵌入只在跨文档块引用、精确定位或后续编辑辅助时按需开启。HTML 注释对 Markdown 渲染无影响，对 AI 阅读干扰最小。
...
写入工具设计：
- 编辑 = 文本锚点查找 + 替换。旧文本为空 = 追加到末尾。新文本为空 = 删除。
- 只支持单块编辑。跨块文本会返回错误，需拆成多次调用。
- 块属性（样式、类型等）在后台处理，不对 AI 暴露，编辑时原样保留。
- 所有写入需 `confirmed=true`。不过该参数完全依赖AI自觉，因此仅作为提示，不起到强制作用。

故意不提供的写入能力：删除、移动、重命名文档、插入附件、修改块属性。这些操作破坏性强，且复杂度远高于文本编辑，当前没有提供 MCP 接口。

### 编辑语义矩阵

| old_text | new_text | 行为 |
|---|---|---|
| 非空 | 非空，`new_text.startswith(old_text)` + 精确匹配 | 在锚点块后插入新块 |
| 非空 | 非空，其他 | 替换（或子串替换） |
| `""` | 非空 | 末尾追加 |
| 非空 | `""` | 删除块 |
| `""` | `""` | 报错 |

锚点匹配插入的设计借鉴了 Claude Code Edit 工具：AI 传入 `old_text` 和 `new_text`，工具自动判断是替换还是插入——`new_text.startswith(old_text)` 且锚点块内容与 `old_text` 完全一致时，提取后缀内容插入到锚点块之后。这与 AI 编辑文件的心智模型完全一致。

### 块 ID 消歧义编辑

当 `old_text` 很短（如单个词"信息"）时，可能同时匹配多个块。`siyuan_edit_document` 提供可选 `block_id` 参数来消除歧义。

提供 `block_id` 时跳过文本搜索，直接用 ID 定位块，但必须同时通过四项验证：
1. 块 ID 存在且内容非空
2. 块属于当前文档（`root_id` 匹配）
3. 块不是数据库块（`type=av`）
4. `old_text` 是块内容的子串

四项全部通过才执行编辑。`block_id` 的唯一用途是歧义消解——其他情况不需要它。当 `old_text` 已能唯一定位时传 `block_id` 是冗余但无害的（跳过 SQL 搜索直接验证）。`block_id` 的语义不随编辑模式（替换/插入/删除）变化。

### 块样式属性（IAL）静默保留

思源块的块样式（背景色、信息/警告卡片等）存储在 `blocks.ial` 字段中。`siyuan_edit_document` 调用 `update_block` 时如不传 IAL，思源内核会重置样式。
...
**About 文档覆盖机制**：内置模板含版本标识（`<!-- template_version: 2 -->`）。refresh 时对比现有文档是否包含相同版本标识——匹配则跳过，不匹配则用新模板覆盖。用户自行编辑后版本标识被破坏，不会被意外覆盖。
...
**步骤 5：搜索** — 需要跨笔记本精确检索时，调用 `siyuan_find_documents`。API-only 召回 → 隐私过滤 → 元数据补全 → 返回按文档分组的命中结果。

**步骤 6：阅读** — AI 判断需要深读某篇文档时，调用 `siyuan_read_document`。先返回大纲（标题→块位置），再按块窗口返回正文。长文档用 `block_start=N` 翻页继续。附件自动提取到 `ai_workspace/`。

**步骤 7：写入**（可选）— 用户明确要求写入时，AI 调用 `siyuan_create_document` 或 `siyuan_edit_document`。写前创建数据快照，快照成功写入才生效。写入后 pushMsg 通知思源前台。
