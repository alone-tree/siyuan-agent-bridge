...
AI 编程工具的核心工具是列出文件、搜索、读文件、编辑、写文件。思源桥一一对应，并额外提供一个文档级管理工具：
...
| `siyuan_doc_manage` | renameDocByID + moveDocsByID + removeDocByID + createDocWithMd + exportMdContent | 文档树管理：改名、移动、删除、复制、导出 |
...
**语言与命名策略**：同一个工作空间只维护一套系统文档。查找时兼容中文名（`思源桥`）和英文名（`SiYuan Bridge`），以及已知的旧名（`思源代理桥`、`SiYuan Agent Bridge`、`关于Siyuan Agent Bridge`）。内部使用稳定 key（`ai_guide`、`workspace_index`、`about`、`privacy_rules`），不依赖显示名称。系统文档缺失时按当前语言创建（中文环境创建中文名，非中文环境创建英文名）；已存在的沿用已有名称，不自动重命名。
...
**步骤 1：用户触发** — 用户说"帮我查一下笔记里关于 XX 的内容"，触发 `siyuan-bridge` skill。
...
└── skills/             # Skill 定义（siyuan-bridge + siyuan-index-builder）
```
...
- `rename`、`move`、`delete` 会在操作前创建思源工作空间快照，成功后刷新索引。
- `copy` 通过导出源文档 Markdown 后调用 `createDocWithMd` 创建副本；源文档只需可读，但创建副本仍需 `confirmed=true` 和快照。
- `export` 将 Markdown 导出到 `ai_workspace/exports/`，不修改思源，不需要快照。
- 隐私规则兼容旧 `Hide=yes/no`；新增 `Permission` 列时支持 `hidden/read_only/read_write`。
