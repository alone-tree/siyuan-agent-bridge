...
| `关于 SiYuan Agent Bridge` / `About SiYuan Agent Bridge` | 不存在则创建；模板版本更新时覆盖 | 给人看的工具说明和开发者消息，精简概括核心思想和用法 |
...
siyuan_start
    ...
    ├── 检查 关于 SiYuan Agent Bridge / About SiYuan Agent Bridge
    │   ├── 不存在 → 用内置模板创建
...
| 思源系统笔记本 / 关于 SiYuan Agent Bridge 或 About SiYuan Agent Bridge | 系统维护，模板版本更新时覆盖 |
