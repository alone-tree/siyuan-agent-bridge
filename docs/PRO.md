...
隐私规则完全由人类在思源系统笔记本 `思源代理桥` / `SiYuan Agent Bridge` 中的 `隐私规则` / `Privacy Rules` 文档维护，使用 Markdown 表格。AI 无法读取、搜索、编辑或总结该文档——它被系统硬编码隔离。
...
系统笔记本 `思源代理桥` / `SiYuan Agent Bridge` 及其文档不能被隐藏。`siyuan_create_document` 拒绝创建隐私规则文档路径。
...
- [x] 系统笔记本 `思源代理桥` / `SiYuan Agent Bridge` 自动创建和维护，含四份系统文档
...
| 思源 `思源代理桥/AI Guide` | 思源文档 | 思源内部 | 思源同步 | 工作空间级主副本，用户在思源 UI 中编辑 |
| 思源 `思源代理桥/Workspace Index` | 思源文档 | 思源内部 | 思源同步 | 工作空间级语义导航索引，AI 用现有写入工具维护 |
...
| `关于思源代理桥` / `About SiYuan Agent Bridge` | 不存在则创建；模板版本更新时覆盖 | 给人看的工具说明和开发者消息，精简概括核心思想和用法 |
...
- 查找系统笔记本时，同时兼容中文名 `思源代理桥` 和英文名 `SiYuan Agent Bridge`。只要找到任意一个，就使用现有笔记本，不因为当前系统语言不同而新建或重命名。
...
# 关于思源代理桥 / About SiYuan Agent Bridge
...
- 创建或更新语义索引时，把结果写入系统笔记本的工作空间索引文档。中文环境默认路径是 `思源代理桥/工作空间索引`，英文环境默认路径是 `SiYuan Agent Bridge/Workspace Index`；如果已有另一语言名称，则使用已有文档。
...
**隐藏规则保护**：系统笔记本不能被隐藏。如果用户尝试隐藏 `思源代理桥` / `SiYuan Agent Bridge` 笔记本，或隐藏其下的任一系统文档、子文档，MCP 应拒绝并提示：这是系统笔记本，隐藏后启动包和隐私规则会失效；如果确实有私密内容，应先把内容移动到其他笔记本再隐藏。
...
| 思源系统笔记本 / 关于思源代理桥 或 About SiYuan Agent Bridge | 系统维护，模板版本更新时覆盖 |
...
| 单块删除 | 中。误删代价高，空块语义复杂 | 中 | 已支持 `new_text=""` 清空单块；跨块删除不支持；留空块而非真删除（待优化） |
...
**已知限制**：`update_block(id, "")` 将块内容设为空字符串，块仍存在于 `blocks` 表中（变为不可见空块）。块计数不变。若后续需要真正的块删除，可封装思源 `/api/block/deleteBlock` 接口，编辑流程在 `new_text=""` 时分流到 `delete_block`。
...
PROMPT_FOR_AI_INSTALL.md       # 用户可复制给 AI Agent 的提示词
install.bat                    # Windows 优先的一键安装/初始化脚本
doctor.bat                     # 诊断脚本
