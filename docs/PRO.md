...
→ 已升级为问题 11 的系统笔记本方案：在 `思源代理桥` 笔记本中维护 `Guide` 和 `Index`，让它们随思源工作空间切换；本地 `guide.md/index.md` 降级为兼容缓存。详见问题 11。
...
| 思源 `思源代理桥/Guide` | 思源文档 | 思源内部 | 思源同步 | 计划中的工作空间级主副本，用户在思源 UI 中编辑（见问题 11） |
| 思源 `思源代理桥/Index` | 思源文档 | 思源内部 | 思源同步 | 计划中的工作空间级语义导航索引，AI 用现有写入工具维护 |
...
**当前结论**：在思源中维护一个名称固定的系统笔记本，例如 `思源代理桥`（可兼容英文 `SiYuan Agent Bridge`）。`siyuan_start` 内部确保这个笔记本存在，并至少确保其中有 `/Guide` 文档。`/Index` 文档可以不存在；不存在时启动包提示 AI 可在用户确认后创建导航索引。
...
siyuan_start
    │
    ├── 查找固定名称笔记本：思源代理桥 / SiYuan Agent Bridge
    ├── 不存在则调用 /api/notebook/createNotebook 创建
    ├── 检查 /Guide，不存在则用 createDocWithMd 创建默认 Guide
    ├── 读取 /Guide
    ├── 读取 /Index；不存在则返回“尚未创建导航索引”的提示
    └── 刷新并返回普通知识库概览
```

**Index 写入方式**：不新增 `siyuan_write_index` MCP 工具。AI 使用现有写入工具即可：

- 第一次创建 `/Index`：调用 `siyuan_create_document`，目标笔记本为 `思源代理桥`，路径固定为 `/Index`。
- 后续更新 `/Index`：调用 `siyuan_edit_document` 替换正文。
- 所有写入继续走现有快照保护和 `confirmed=true` 机制。

**Skill 规则**：

- 创建或更新语义索引时，把结果写入 `思源代理桥/Index`。
- 整理、总结、重建用户知识库索引时，不要把 `思源代理桥` 笔记本当作用户原始知识材料。
- `思源代理桥/Guide` 是使用规则，`思源代理桥/Index` 是导航索引；二者不是用户原始资料。
- 如果普通搜索命中系统笔记本，AI 应先判断当前任务是否真的需要系统资料；多数用户知识问答不应引用它。

**隐藏规则保护**：系统笔记本不能被隐藏。如果用户尝试隐藏 `思源代理桥` 笔记本，或隐藏其下的 `/Guide`、`/Index`、子文档，MCP 应拒绝并提示：这是系统笔记本，隐藏后启动包会失去 Guide/Index；如果确实有私密内容，应先把内容移动到其他笔记本再隐藏。

**与本地文件的关系**：

| 文件 | 新角色 |
|------|------|
| 思源 `思源代理桥/Guide` | 工作空间级主副本，用户可在思源 UI 中直接编辑 |
| 思源 `思源代理桥/Index` | 工作空间级语义导航索引，AI 用现有写入工具创建/更新 |
| `knowledge_base/guide.md` | 兼容缓存或开发 fallback，长期不再作为主副本 |
| `knowledge_base/index.md` | 兼容缓存或旧流程 fallback，长期不再作为主副本 |
...
这个方案的关键价值是：Guide/Index 随思源工作空间切换；MCP 工具数量不增加；系统笔记本保持可见但通过 Skill 避免污染语义索引。
...
**当前结论**：不新增 `siyuan_write_index`。改为把语义索引主副本放在思源系统笔记本 `思源代理桥/Index` 中，并要求 AI 使用现有 `siyuan_create_document` / `siyuan_edit_document` 维护它。
...
4. Skill 只需说明固定写入位置：`思源代理桥/Index`。

**实现要求**：

- `siyuan_start` 读取 `思源代理桥/Index`，不存在则提示尚未创建。
- `siyuan-index-builder` 生成索引时，创建或更新 `思源代理桥/Index`。
...
| # | 事项 | 改动量 | 效果 | 状态 |
|---|------|:---:|------|:--:|
| **H** | **系统笔记本初始化与 Guide/Index 迁移** | 中 | `siyuan_start` 确保 `思源代理桥/Guide` 存在并读取 `Index`；AI 用现有写入工具维护 Index，不新增 MCP | 待实现 |
