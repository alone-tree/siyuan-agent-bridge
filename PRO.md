...
| **能力暴露层** | 通过 MCP 协议向 AI 暴露只读工具 | `mcp_server.py` (12 tools) | AI 可调用的语义能力 |
...
| 9 | `siyuan_hide` | `scope` + `locator` + `reason`? + `confirmed` | 隐藏笔记本/文档/子树，自动刷新索引 | ✓ |
| 10 | `siyuan_unhide` | `scope` + `locator` + `confirmed` | 取消隐藏，自动刷新索引 | ✓ |
| 11 | `siyuan_temporary_allow` | `scope` + `locator` + `minutes`? (default 60) + `reason`? | 临时开放隐藏内容，不刷新索引，到期自动失效 | ✗ |
| 12 | `siyuan_close_temporary_allow` | 无 | 立即清除所有临时开放规则 | ✗ |
...
隐私层:   siyuan_hide            → 隐藏内容（需 confirmed=true）
         siyuan_unhide          → 取消隐藏（需 confirmed=true）
         siyuan_temporary_allow → 临时开放（有时效性）
         siyuan_close_temporary_allow → 关闭所有临时开放
```
...
- [x] `siyuan-knowledge` SKILL.md 更新：Mandatory Startup 明确 AI 优先使用 index.md 快速路由表，Tool Use 添加 index.md 说明
...
- [x] 单元测试覆盖
- [x] **MCP 隐私工具**（hide/unhide/temporary_allow/close_temporary_allow）：4 个工具暴露到 MCP，confirmed=true 保护
- [x] **`siyuan_refresh_index` 清理 ai_workspace**：每次 refresh 清空临时文件，保留 README.md
- [x] **Bug 修复**：`find_documents()` haystack 补全 alias/memo 字段；`_enrich_search_blocks()` 不再丢弃 FTS 命中但不在 local_docs 中的文档；FTS paths 参数格式修正
- [x] **Skill 打包**：`dist/siyuan-knowledge-skill-latest.zip` 含隐私工具和 index.md 指令

### 待实现

- [ ] **tree.md 增加块数**：当前 SQL 查询已获取全部字段，但 `normalize_documents()` 未计算块数
- [ ] **`siyuan_find_documents` 实时搜索隐私过滤审查**：确认对隐藏内容的过滤在所有搜索路径下都是绝对的
...
**A + B + C + D + E 已完成** (2026-05-02)。A（index.md 纳入启动包）、B（SKILL.md 更新）、C（MCP 隐私工具 4 个）、D（连接失败友好提示）、E（ai_workspace 清理）全部到位。

同时修复了 3 个 Bug：`find_documents()` haystack 缺少 alias/memo、`_enrich_search_blocks()` 静默丢弃 FTS 结果、FTS paths 参数格式。

下一步可选：F（思源Agent笔记本 + 写操作）、G（块 ID 嵌入）、H（附件提取）。
