...
| **能力暴露层** | 通过 MCP 协议向 AI 暴露只读工具 | `mcp_server.py` (9 tools) | AI 可调用的语义能力 |
...
读 API（已有覆盖）                    写 API
─────────────────────────           ─────────────────────────
notebook/lsNotebooks                 notebook/createNotebook      ❌
filetree/listDocsByPath              notebook/renameNotebook      ❌
filetree/getHPathByID                notebook/removeNotebook      ❌
filetree/getIDsByHPath             
block/getBlockKramdown               filetree/createDocWithMd     ✅
block/getChildBlocks                 filetree/renameDocByID       ❌
attr/getBlockAttrs                   filetree/removeDocByID       ❌
search/fullTextSearchBlock           filetree/moveDocsByID        ❌
query/sql                         
export/exportMdContent               block/appendBlock            ⚡
                                      block/prependBlock           ⚡
                                      block/insertBlock            ⚡
                                      block/updateBlock            ⚡
                                      block/deleteBlock            ⚠️
...
图例：✅ = 暴露为 MCP 工具。⚡ = 服务端内部使用（AI 不直接调用）。❌ = 不实现。⚠️ = 通过 edit 工具隐式完成。
...
| API | 理由 |
|-----|------|
| `removeDoc` / `deleteBlock`（独立工具） | 删除通过 `edit_block(new_text="")` 隐式完成。Claude Code 也没有独立的删除工具 |
...
| `prependBlock` / `insertBlock`（独立暴露） | AI 通过 `edit_block` 的文本锚点操作间接完成，不需要知道块位置参数 |
| 整篇覆盖（Write 覆盖现有文档） | 思源文档有子文档树、标签、引用等附加属性，清空重建会丢失这些。只做创建不做覆盖 |

#### 跨文档块引用需要块 ID 嵌入

`siyuan_edit_block` 的文本锚点方案解决了**同一文档内**的增删改。但跨文档块引用场景——在文档 A 中引用文档 B 的某个块 `((block_id "锚文本"))`——AI 必须知道目标块的 ID。
...
| 阶段 | 内容 | 前置依赖 |
|------|------|---------|
| **第一阶段** | `siyuan_edit_block` + `siyuan_create_document` | 无。文本锚点不需要块 ID 嵌入 |
| **第二阶段** | 块 ID 嵌入（`siyuan_read_document` 返回注释） | 仅在跨文档块引用时需要 |
...
Claude Code 允许用户随时回滚到前一次对话的文件状态，原理是每次 Edit/Write 前自动备份原始文件内容。

**对本项目的可行性**：`siyuan_edit_block` 在匹配阶段已经知道要改哪些块和改之前的内容（`old_text` 匹配结果 → `block_id` + 原文），记录快照的成本为零。但回滚不能用 Markdown 导出重建，因为思源不是纯文本文档：

| Markdown 导出重建会丢失 | 后果 |
|--------------------------|------|
| 块 ID | 其他文档的 `((block_id "text"))` 引用全部断裂 |
| 子文档树 | 父文档下的子文档变成孤儿 |
| 块属性（`custom-*`） | 自定义属性丢失 |

因此快照必须记录到**块级别**，回滚时通过 `updateBlock` 逐块恢复原内容，不改变块 ID：

```
每次 siyuan_edit_block 自动记录:
  snapshot = {
    document_id: "...",
    changes: [
      {block_id: "abc", before: "原文", after: "新文"},
      {block_id: "def", before: "原文", after: "新文"}
    ]
  }
  写入 ai_workspace/snapshots/<session-id>.jsonl

回滚: siyuan_rollback(document_id)
  → 读取最近快照，逆序逐条 updateBlock(before)
  → pushMsg("已回滚 [文档名] 的 N 个块")
```

AI 只需要调用 `siyuan_rollback(document_id)`，不需要知道改过哪些块。块 ID、子文档树、跨文档引用全部保持完整。

**优先级**：写入功能稳定后再考虑。初期 `siyuan_edit_block` 的 `old_text` 本身就是一个最小的操作证据——即使没有自动快照，AI 也可以通过记录自己传过的 `old_text` 来手动"撤销"。
...
- 不做 delete 独立工具：删除只能通过 edit_block(new_text="")
...
| # | 事项 | 改动量 | 效果 |
|---|------|:---:|------|
| **F** | **写入功能第一阶段**：`siyuan_edit_block` + `siyuan_create_document` | 中 | AI 能在思源文档中增删改内容，以文本锚点定位（无需块 ID 嵌入） |
...
下一步：F（写入功能第一阶段 — `siyuan_edit_block` + `siyuan_create_document`）、G（块 ID 嵌入）、H（`siyuan_write_index`）、I（`siyuan_temporary_allow` 修复）。
...
#### F. 写入功能第一阶段：`siyuan_edit_block` + `siyuan_create_document`
...
**步骤**：
1. `client.py` 新增 `append_block()`、`update_block()`、`insert_block()`、`create_doc_with_md()` 方法
2. `mcp_server.py` 新增 `siyuan_edit_block(document_id, old_text, new_text, confirmed)` —— 文本锚点搜索 + 块操作
