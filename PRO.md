...
- [ ] **MCP 隐私工具**（hide/unhide/allow/close）：CLI 已实现，需暴露到 MCP
- [ ] **MCP 隐私工具**（hide/unhide/allow/close）：CLI 已实现，需暴露到 MCP
- [ ] **`siyuan_refresh_index` 清理 ai_workspace**：每次 refresh 清空临时文件
...
**A + B 已完成** (2026-05-02)。接下来做 **C + D**（MCP 隐私工具 + 连接失败友好提示），改动量中等，让日常使用更顺手。
