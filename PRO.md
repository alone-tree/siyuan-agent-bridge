...
| 5 | `siyuan_read_document` | `document_id` + `chunk`? (default 0) + `max_chars`? + `extract_attachments`? (default false) | 返回大纲；短文档全文；长文档分 chunk；可选提取附件到 ai_workspace | ✓ |
...
| 9 | `siyuan_privacy` | `action` ("hide"\|"unhide") + `scope` + `locator` + `confirmed` + `reason`? | 隐藏或取消隐藏笔记本/文档/子树，自动刷新索引 | ✓ |
| 10 | `siyuan_temporary_allow` | `action` ("open"\|"close") + `scope`? + `locator`? + `minutes`? + `reason`? | open=临时开放隐藏内容（有时效）；close=清除所有临时开放 | ✗ |
