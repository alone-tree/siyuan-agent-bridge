...
**当前结论**：默认保持纯 Markdown 阅读体验。AI 使用笔记时，最常见需求是搜索、理解、总结、改写和局部文本修改，纯 Markdown 的性价比最高。块 ID 增强不是第一阶段必需能力，只有跨文档块引用、诊断复杂结构或用户明确要求精确定位时才值得引入。
...
| # | 事项 | 改动量 | 效果 |
|---|------|:---:|------|
| **G** | **块 ID 嵌入可选模式**（`siyuan_read_document` 返回 `<!-- block:xxx -->` 注释） | 中 | 仅在跨文档块引用或诊断复杂结构时启用，不作为默认阅读返回 |
...
下一步：H（`siyuan_write_index`）、I（`siyuan_temporary_allow` 修复）。G（块 ID 嵌入）降级为可选增强，不再作为写入功能的前置方向。
...
在 `siyuan_read_document` 的可选模式中嵌入 `<!-- block:xxx -->` HTML 注释。默认阅读不启用，避免把复杂块元数据带入普通阅读场景。需要跨文档块引用 `((id "text"))`，或需要诊断复杂结构时，AI 可以提取目标块 ID。写入功能不依赖此项——文本锚点已足够。
