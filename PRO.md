...
| Skill | 触发条件 | 核心指令 |
|-------|----------|---------|
| `siyuan-agent-bridge` | 用户提到思源/知识库/笔记 | 强制调用 siyuan_start 获取启动包 → 按导航定位 → 按需深读 |
...
- `scope=full` 使用块级结果生成正文片段，避免只返回文档标题导致 AI 无法判断命中原因。
...
4. 对每个候中文档应用 `siyuan.ignore.local.json` 和 `siyuan.allow.local.json` 规则；未通过过滤的结果直接丢弃，不返回给 AI。
5. 用 `docs.jsonl` 和 `notebooks.json` 补全文档字数、块数、更新时间、笔记本名称等元数据；它们不参与搜索匹配。
...
- 子树隐藏不能只依赖已过滤的 `docs.jsonl`，因为隐藏根文档可能已从本地索引移除；需要用 API 返回的 `path` 判断文档是否位于隐藏根文档下。
...
**三-I（tree.md 块数）已完成** (2026-05-02)。扩展为统计指标全面重构：新增 `block_count` + `char_count`，删除死代码 `index_word_count`，索引刷新从 424 次 HTTP 调用优化为 2 次 SQL 查询（~30s → ~2s）。所有展示同步更新。

**三-J（实时搜索隐私过滤审查）已完成** (2026-05-02)。`_enrich_search_blocks()` 对不在安全索引的文档一律 `continue`，正确性由 `ensure_notebooks_open` 保证。

**三-H（附件提取）已完成** (2026-05-02)。`siyuan_read_document` 自动提取所有 `assets/` 引用文件（图片、PDF、xlsx 等），通过思源 HTTP API 下载到 `ai_workspace/attachments/<doc-id>/assets/`，保留原始文件名和目录结构。Markdown 原文不改动。`siyuan_refresh_index` 自动清理。

下一步可选：F（思源Agent笔记本 + 写操作）、G（块 ID 嵌入）。
