...
5. **写前快照**：每次编辑或创建文档前，先调用思源仓库快照 API 备份整个工作空间。第一阶段不提供 AI 自动回滚工具；如果出现严重问题，用户自行用思源快照恢复工作空间。
...
| 阶段 | 内容 | 前置依赖 |
|------|------|---------|
| **第一阶段** | `siyuan_edit_document` + `siyuan_create_document` | 无。文本锚点不需要块 ID 嵌入 |
...
| # | 事项 | 改动量 | 效果 |
|---|------|:---:|------|
| **F** | **写入功能第一阶段**：`siyuan_edit_document` + `siyuan_create_document` | 中 | AI 能在思源文档中增删改内容，以文本锚点定位（无需块 ID 嵌入） |
...
下一步：F（写入功能第一阶段 — `siyuan_edit_document` + `siyuan_create_document`）、G（块 ID 嵌入）、H（`siyuan_write_index`）、I（`siyuan_temporary_allow` 修复）。
