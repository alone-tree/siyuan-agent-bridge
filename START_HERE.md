...
7. Read full documents only when needed, through `siyuan_read_document` or `python -m source_code read <doc-id>`.
8. Do not refresh/rebuild indexes unless the user asks, the index is missing, or the index is clearly stale.
